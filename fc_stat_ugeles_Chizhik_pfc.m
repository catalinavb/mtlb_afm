...
pfc_file='ugel-PAH9_25C.001.pfc';
...
saturation = 0; % z limit in case saturation occurs (0 if not saturated)
...
hold on
plot(x_cont(contactPointIndex), y_cont(contactPointIndex),'*g')
...
h=0;
for e=1:length(EM)
    if isempty(EM(e).modulus1)==0
        h=h+1;
EM1(h) =EM(e).modulus1;
Ef_hertz(h) = EM(e).modhertz;
Ef_sned(h) = EM(e).modsned;
delta_EM1(h)=EM(e).err1;
    end
end
% h=1;
% for e=1:length(EM)
